...
const activity = {
  ...
  mergeTable: async (table, defaultValue) => {
    const batch = 200;
    try {
      ...
      for (let i = 0; i < nIter; i++) {
        await activity.loadSrcData(table, i * batch, batch).then((rows) => Promise.all(rows.map((row) => {
          return model.findOne({ _id: row._id }).then((exists) => {
            ...
            // set default values.
            Object.keys(defaultValue).forEach((key, i) => {
              document[key] = document[key] || defaultValue[key];
            });
...
module.exports = async (req, res) => {
  ...
  const { tables, defaultValues } = req.body;
  return niv.check().then((matched) => {
    if (!matched) throw niv.errors[0];
  }).then(async () => {
    for (let i = 0; i < tables.length; i++) {
      const defaultValue = defaultValues && typeof defaultValues === 'object' && defaultValues[i] ? defaultValues[i] : {};
      await activity.mergeTable(tables[i], defaultValue);
    }
    return res.json({
      status: true,
      message: 'success',
